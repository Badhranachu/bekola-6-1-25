...
router.get("/cart", async (req, res) => {
  try {
    ...
    if (!cartDetails || !cartDetails.products || cartDetails.products.length === 0) {
      return res.render("user/cart", { cartItems: [], totalPrice: 0, user: req.session.user });
    }
...
router.get('/orders', async (req, res) => {
  ...
  try {
    const orders = await productHelpers.getOrdersByUserId(userId);

    if (!orders || orders.length === 0) {
      console.log("No orders found for this user.");
      return res.render('user/orders', { orders: [], user: req.session.user }); // Send empty array
    }

    for (const order of orders) {
      const product = await productHelpers.getProductById(order.productId);
      order.productName = product?.name || "Unknown Product";
      order.productImage = product?.image || "/default-image.jpg";
    }

    orders.forEach(order => {
      order.orderDateFormatted = moment(order.orderDate).format('MMMM Do YYYY, h:mm:ss a');
    });
    ...
    console.error('Error fetching orders:', error);
      res.status(500).render('user/orders', { orders: [], user: req.session.user, error: "Something went wrong while fetching your orders." });
  }
});
...
router.post('/admin-signup', async (req, res) => {
  console.log("Entered admin signup");

  const { Name, Email, Password } = req.body;

  try {
      let adminExist = await db.get().collection(collection.ADMIN_COLLECTION).findOne({ Email });

      if (adminExist) {
          return res.render('admin/admin-login', { errorMessage: "Admin already exists" }); // ❌ Error Message
      }
      ...
      await db.get().collection(collection.ADMIN_COLLECTION).insertOne(newAdmin);
      
      // ✅ Redirect to login with success message
      return res.render('admin/admin-login', { successMessage: "Admin registered successfully" });
...
router.get('/admin', (req, res) => {
  res.render('admin/admin-login', { admin: true }); // Ensure admin=false
});
...
router.post('/admin-login', async (req, res) => {
  ...
  try {
    ...
    if (!admin) {
      return res.render('admin/admin-login', { admin: false, errorMessage: 'Admin not found' });
    }

    const passwordMatch = await bcrypt.compare(Password, admin.Password);
    if (!passwordMatch) {
      return res.render('admin/admin-login', { admin: false, errorMessage: 'Invalid email or password' });
    }
...
router.get('/view-total', async (req, res) => {
  ...
  try {
      ...
      // Render the product view with fetched products
      res.render('admin/view-products', { admin: true, products });
  } catch (error) {
      console.error("Error fetching products:", error);
      res.status(500).json({ message: "Internal server error" });
...
router.get('/allorders', async (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin'); // Redirect if no admin session
  }
